#!/bin/bash
JSON_DATA="$(mysql -B dsubs_prod -e 'select * from kill_records where created_at < UTC_TIMESTAMP - INTERVAL 30 minute and simulator_id like "main%" order by id desc limit 250;' | csvjson)"
